<template>
  <div v-loading="loading">
    ...
    <Category :isCategory="isCategory" :categoryList="categoryList" @close="closeCategoryFunc"></Category>
  </div>
</template>
    ...
    getData(id) {
      ...
      CouponApi.couponDetail(
        ...
        .then(res => {
          ...
          obj.couponType = res.data.couponType;
          self.form = formatModel(self.form, obj);
          if (self.form.applyRange == 20) {
            self.excludeIds = res.data.productShowIds;
            for (let i = 0; i < self.excludeIds.length; i++) {
              self.excludeIds[i] = parseInt(self.excludeIds[i]);
            }
            self.productList = res.data.productList;
          }
          if (self.form.applyRange == 30) {
            self.categoryList = res.data.categoryList;
            self.categoryList.first.forEach((item, index) => {
              self.categoryListFirst.push(item.categoryId);
            });
            self.categoryList.second.forEach((item, index) => {
              self.categoryListSecond.push(item.categoryId);
            });
          }
          self.form.activeTime = [];
          self.form.activeTime[0] = res.data.startTime;
          self.form.activeTime[1] = res.data.endTime;
          //self.form.discount = self.form.discount / 10;
          self.loading = false;
        })
        .catch(error => {
          console.log(error);
